...
#                     # Creating non-overlapping ranges for age or numeric data
#                     bin_edges = np.linspace(cluster_group[col].min(), cluster_group[col].max(), self.k + 1)
#                     bin_labels = [f"{int(bin_edges[i])} - {int(bin_edges[i+1])}" for i in range(self.k)]
#                     self.df.loc[self.df['Cluster'] == cluster_id, col] = pd.cut(cluster_group[col], bins=bin_edges, labels=bin_labels)
...
#             bin_edges = np.linspace(df[best_col].min(), df[best_col].max(), self.k + 1)
#             bin_labels = [f"{int(bin_edges[i])} - {int(bin_edges[i+1])}" for i in range(self.k)]
#             df[best_col] = pd.cut(df[best_col], bins=bin_edges, labels=bin_labels)
#         else:
...
# # Create anonymizer
# anonymizer = Multi(k=5, df=df, qi=qi_list)
...
relaxed_df.to_csv('relaxed_anonymized_data.csv', index=False)

# Apply greedy anonymization
greedy_df = anonymizer.greedy()
print(greedy_df.head())
greedy_df.to_csv('greedy_anonymized_data.csv', index=False)
